...
import { Upload, Button, Input, Select, Form, message, Card, UploadProps, UploadFile, Progress, Collapse, List } from 'antd';
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex justify-center items-center">
      <Card className="w-full max-w-2xl shadow-lg">
        ...
        <Collapse activeKey={isCollapsed ? [] : ['1']} onChange={() => setIsCollapsed(!isCollapsed)}>
          <Panel header="Yükleme Bilgileri ve Dosya Yükleme Alanı" key="1">
            <Form layout="vertical" onFinish={handleSubmit}>
              ...
              <Form.Item label="Açıklama" rules={[{ required: true, min: 10, message: 'Açıklama en az 10 karakter olmalıdır.' }]}> 
              ...
              <Form.Item>
                <Button type="primary" htmlType="submit" className="w-full">Yükle</Button>
              </Form.Item>
            </Form>
          </Panel>
        </Collapse>
        <Collapse className="mt-4" activeKey={isFilesPanelOpen ? ['1'] : []} onChange={() => setIsFilesPanelOpen(!isFilesPanelOpen)}>
          <Panel header="Yüklenen Dosyalar" key="1">
            <List
              ...
              renderItem={(file) => (
                ...
                >
                  <List.Item.Meta
                    title={file.name}
                    description={<Progress percent={uploadProgress[file.uid] || 0} />}
                  />
                </List.Item>
              )}
            />
